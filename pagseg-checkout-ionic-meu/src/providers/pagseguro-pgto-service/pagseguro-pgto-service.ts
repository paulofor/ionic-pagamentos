...
export class PagseguroPgtoServiceProvider {
  ...
  constructor(public http: HttpClient, private storage: Storage, private varGlobais: VarGlobalProvider, private datepipe: DatePipe) {
  ...
  buscaParcelas() {
    PagSeguroDirectPayment.getInstallments({
      amount: '100',              //valor total da compra (deve ser informado) -> valor
      brand: this.dados.bandCard, //bandeira do cartão (capturado na função buscaBandeira)
      maxInstallmentNoInterest: 3,  // maximo de parcelas
      success: response => {
